...
var ReactionForm = React.createClass({
  ...
  handleSubmit: function(e) {
    ...
    this.props.onReactionSubmit({author: author, text: text}, this.props.commentNum)
    this.setState({author: '', text: ''});
  },
  render: function() {
    return (
      <div className="col-md-10">
        <form className="reactionForm" onSubmit={this.handleSubmit}>
          <input
...
var ReactionList = React.createClass({
  ...
  render: function() {
    return (
      <div>
        {this.props.data.length > 0 ? <h4>REACTions</h4> : <div className="panel panel-default"><h4>No REACTions Yet!</h4></div>}
...
var Comment = React.createClass({
  ...
  showReactionForm: function() {
    this.setState({ showReactionForm: true });
  },
  hideReactionForm: function() {
    this.setState({ showReactionForm: false });
  },
  onReactionSubmit: function(reaction, commentIndex) {
		...
		return (
          ...
          <div class="row">
            ...
            <small>
              ...
              { this.state.showReactionForm ? <button className="btn btn-info btn-sm" onClick={this.hideReactionForm}>Hide <span className="glyphicon glyphicon-comment" /></button> : <button className="btn btn-info btn-sm" onClick={this.showReactionForm}>Add REACTion <span className="glyphicon glyphicon-pencil" /></button> }
            </small>
          </div>
          <div className="row">
            ...
            <div className="col-md-5">
              ...
              { this.state.showReactionForm ? <ReactionForm onReactionSubmit={this.onReactionSubmit} commentNum={this.props.index} /> : null }
            </div>
            <div className="col-md-5">
            </div>
          </div>
...
var CommentList = React.createClass({
  ...
  handleAddReaction: function(reaction, commentIndex) {
    console.log('add reaction %s by %s: to comment %d', reaction["text"], reaction["author"], commentIndex);
    var comment = this.props.data[commentIndex];
    comment["reactions"].push(reaction);
    this.props.addReaction(comment, commentIndex);
  },
	render: function() {
    return (
    	<div>
        {this.props.data.map((comment, commentIndex) =>
          <div className="row"> 
            ...
            <div className="comment col-md-1">
                ...
                <Comment author={comment.author} index={commentIndex} likes={comment.likes} dislikes={comment.dislikes} handleLike={this.handleLike} handleDislike={this.handleDislike} key={comment.key} reactions={comment.reactions} deleteReaction={this.deleteReaction} handleAddReaction={this.handleAddReaction} >{comment.text}</Comment>
              </div>
...
var CommentBox = React.createClass({
  ...
  addReaction: function(comment, ci) {
    var comments = this.state.data;
    this.setState(state => { 
      state.data[ci] = comment;
      return {data: state.data}
    });
    $.ajax({
      url: this.props.url + "?_=" + ci,
      dataType: 'json',
      type: 'PUT',
	...
	render: function() {
		return (
			<div className="commentBox">
				<h1>Comments 🐳</h1>
				<div className="row commentList"><CommentList data={this.state.data} deleteComment={this.deleteComment} deleteReaction={this.deleteReaction} likeComment={this.likeComment} dislikeComment={this.dislikeComment} addReaction={this.addReaction} /></div>
